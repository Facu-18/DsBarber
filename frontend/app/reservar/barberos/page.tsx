...
export default function SeleccionarBarbero() {
  return (
    <div className="w-full max-w-5xl mx-auto bg-white/10 backdrop-blur-md border border-white/20 rounded-3xl shadow-2xl p-6 sm:p-8 text-white">
      ...
      <div className="flex flex-col md:flex-row gap-6 sm:gap-8 mb-10 sm:mb-12">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-3 sm:gap-4 mb-4">
            <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full overflow-hidden flex items-center justify-center bg-white">
              <Image src="/DS_Barbershop_Logo.svg" alt="Logo" width={150} height={60} priority />
            </div>
            <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Ds Barberstudio</h1>
          </div>
        ...
        <div className="flex-1">
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 sm:p-6 border border-white/15 shadow-sm">
            <Map />
            <Link
              ...
              target="_blank"
              className="block text-sm text-white hover:text-white transition-colors duration-200"
            >
              Bartolomé Hidalgo 1669, Córdoba Parque Liceo III
            </Link>
            <p className="text-sm text-white mt-1">+54 9 3512291106</p>
          </div>
        </div>
      </div>

      {/* Sección de InfoService y SelectBarber */}
      <Suspense>
        <InfoService />
      </Suspense>

      <div className="mt-8">
        <Suspense fallback={<p className="text-center text-white/70">Cargando barberos...</p>}>
          <SelectBarber />
        </Suspense>
      </div>
