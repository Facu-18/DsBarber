...
export class ReservationEmail {
  static sendConfirmationEmail = async (user: EmailType) => {
    ...
    const link = `${process.env.FRONTEND_URL}/reserva?email=${encodeURIComponent(user.email)}&date=${user.date}&time=${encodeURIComponent(user.time)}`;
      ...
      html: `
          <div style="max-width: 600px; margin: auto; padding: 24px; font-family: Arial, sans-serif; background-color: #ffffff; border: 1px solid #e0e0e0; border-radius: 12px;">
              ...
              <p style="font-size: 13px; color: #6b7280;">
                Si no hiciste esta reserva o tienes alguna pregunta, podés contactarnos al teléfono +54 9 3512291106.
              </p>
          </div>
      `,
    });
